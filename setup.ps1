...
scoop install 7zip actionlint aria2 aspell azure-cli azure-functions-core-tools azure-kubelogin azure-ps bat bazel bazel-buildtools biber bottom bun ccache clj-deps clojure-lsp cmake composer curl dart deno deno-lint defold doxygen dust elixir elm emacs emscripten erlang fd ffmpeg flutter fzf gcc git github gitlab-release-cli gitlab-runner glab glslang go godot golangci-lint graalvm graalvm-oracle-jdk gradle gradle-bin grep gzip hadolint haskell haskell-cabal helix jdtls jq julia kafka kafka-exporter kotlin kotlin-language-server kotlin-native ktlint kube-linter lazygit less libxml2 llvm love lua lua-language-server luacheck luajit luarocks make mambaforge maven meson miktex mingw mprocs msys2 neovim nim ninja nodejs nodejs nu nu openjdk php poetry premake protolint psgithub quick-lint-js racket redis redis-tui revive ripgrep ripgrep ruby rustup sbt scala scala-cli sccache search-multisource sed solidity spark starship sudo swift teal terminal-icons temurin-jdk terraform tflint tidy touch tree-sitter tree-sitter-langs unzip vale vcpkg wget xmake xmllint yt-dlp zig zookeeper
