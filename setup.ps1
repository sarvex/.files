...
scoop install actionlint aria2 aspell bat bazel bazel-buildtools biber bottom ccache clj-deps clojure-lsp cmake composer conan crystal curl defold deno deno-lint doxygen dust elixir elm emacs emscripten erlang fd ffmpeg fzf glslang go golangci-lint godot graalvm gradle-bin grep gzip hadolint haskell helix jq julia kotlin kotlin-language-server kotlin-native ktlint kube-linter lazygit less libxml2 llvm love lua lua-language-server luacheck luajit luarocks make mambaforge meson miktex mingw mprocs msys2 neovim-nightly nim ninja nodejs openjdk php premake protolint puppet puppet-bolt quick-lint-js racket revive ripgrep ripgrep ruby rustup sbt scala scala-cli sccache search-multisource sed starship sudo swift teal terminal-icons terraform tflint tidy touch tree-sitter tree-sitter-langs UbuntuMono-nerd-font unzip vale vcpkg wget xmake xmllint yt-dlp
